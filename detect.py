...
def args_parse():
    ...
    parser.add_argument('-model_path', default="./mod_097/Epoch11-Total_Loss14.7709-Val_Loss15.3326.pth", type=str, help="model path")
    # parser.add_argument('-model_path', default="./mod_swish_bifpn/", type=str, elp="model path")
    parser.add_argument('-classes_path', default='./model_data/voc_classes.txt', type=str, help='classes path')
    parser.add_argument('-anchors_path', default='./voc0712_anchors.txt', type=str, help='anchors_path')
...
def detect_image(image):
    ...
    net = YoloBody(len(anchors[0]), len(class_names))
    net.eval()
...
if __name__ == "__main__":
    ...
    if is_image:
        ...
        # image = np.array(detect_image(image))
        # image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        result = detect_image(image)
        cv2.namedWindow("result", cv2.WINDOW_NORMAL)
        cv2.imshow("result", image)
        cv2.imwrite("./img/street-det.jpg", image)
        cv2.waitKey()
