...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, img_size, label_smooth, cuda=True):
    ...
    def forward(self, input, targets=None):
        ...
        stride_h = self.img_size[1] / in_h  # 这个结果就是感受野
        stride_w = self.img_size[0] / in_w

        # 把先验框的尺寸调整成特征层大小的形式
        # 计算出先验框在特征层上对应的宽高
        # 原始锚框除以现在的步长，就是现在特征层的锚框
        ...
        # permute将tensor的维度多个换位 contiguous返回一个内存连续的有相同数据的tensor，如果原tensor内存连续，则返回原tensor
        # view只能对连续的tensor进行换位，transpose只能交换两个维度
        
        # 对prediction预测进行调整
        conf = torch.sigmoid(prediction[..., 4])  # Conf  序号为4 代表先验框内部是否包含物体
        pred_cls = torch.sigmoid(prediction[..., 5:])  # Cls pred.  序号为5 代表先验框内部物体的类别

        # 找到哪些先验框内部包含物体
        # self.get_target 获取网络应该有的结果，就是目前特征层的真实位置
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):  # 真实物体，锚框宽高，特征图宽、高，阈值
        # 计算一共有多少张图片
        bs = len(target)
        # 获得先验框
        anchor_index = [[0, 1, 2], [3, 4, 5], [6, 7, 8]][self.feature_length.index(in_w)]
        subtract_index = [0, 3, 6][self.feature_length.index(in_w)]  # index()函数用于从列表中找出某个值第一个匹配项的索引位置
        # 创建全是0或者全是1的阵列
        ...
        for b in range(bs):
            ...
            # 计算出属于哪个网格
            gis = torch.floor(gxs)  # 床函数，返回一个新张量，包含输入input张量每个元素的floor，即不小于元素的最大整数
            gjs = torch.floor(gys)
    ...
    def get_ignore(self, prediction, target, scaled_anchors, in_w, in_h, noobj_mask):  # 负样本，去除多余的预测框
        bs = len(target)
        # index()函数用于从列表中找出某个值第一个匹配项的索引位置
        anchor_index = [[0, 1, 2], [3, 4, 5], [6, 7, 8]][self.feature_length.index(in_w)]
        scaled_anchors = np.array(scaled_anchors)[anchor_index]  # index()函数用于从列表中找出某个值第一个匹配项的索引位置
        # 先验框的中心位置的调整参数
