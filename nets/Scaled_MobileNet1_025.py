...
class YoloBody(nn.Module):
    def __init__(self, num_anchors, num_classes):
        ...
        self.conv1 = Conv(256, 128, 1)
        self.conv11 = Conv(128, 256, 3)
        self.conv12 = Conv(256, 128, 1)
        self.SPP = SPP()
        self.conv2 = Conv(640, 128, 1)
        self.conv21 = Conv(128, 128, 3)

        self.conv22 = Conv(256, 128, 1)

        self.upsample1 = Upsample(128, 64)  # 卷积+上采样  Upsample(输入，输入/2)  上面的输出就是这里的输入
        self.conv_for_P4 = Conv(128, 64, 1)  # （输入，输入/2，卷积核）
        self.make_five_conv1 = five_csp_conv([64, 128], 128)  # （[输出，输出*2]，输入）  上面的上采样输出就是这里的输入

        self.upsample2 = Upsample(64, 32)  # 卷积+上采样  Upsample(输入，输入/2)  上面的输出就是这里的输入
        self.conv_for_P3 = Conv(64, 32, 1)  # （输入，输入/2，卷积核）
        self.make_five_conv2 = five_csp_conv([32, 64], 64)  # （[输出，输出*2]，输入）  上面的上采样输出就是这里的输入
        ...
        self.yolo_head3 = yolo_head([64, final_out_filter2], 32)  # 第一个yolo_head  （[2*输入，通道数]，输入）

        self.down_sample1 = Conv(32, 64, 3, stride=2)  # （输入，输入/2，卷积核，步长）
        self.make_five_conv3 = five_csp_conv([64, 128], 128)  # （[输出，输出*2]，输入）
        # 3*(5+num_classes)=3*(5+20)=3*(4+1+20)=75
        # 每个网格(特征)点的3个预选框*（5个预测信息（4个参数调整先验框获得预测框1个是判断是否包含物体）+ num_classes的值是预测框中属于(每一个)类的概率）
        final_out_filter1 = num_anchors * (5 + num_classes)  # num_classes是要检测的类的个数（我的数据集类别数是6）
        self.yolo_head2 = yolo_head([128, final_out_filter1], 64)  # 第二个yolo_head  （[2*输入，通道数]，输入）

        self.down_sample2 = Conv(64, 128, 3, stride=2)  # （输入，输入/2，卷积核，步长）
        self.make_five_conv4 = five_csp_conv([128, 256], 256)  # （[输出，输出*2]，输入）
        # 3*(5+num_classes)=3*(5+20)=3*(4+1+20)=75
        # 每个网格(特征)点的3个预选框*（5个预测信息（4个参数调整先验框获得预测框1个是判断是否包含物体）+ num_classes的值是预测框中属于(每一个)类的概率）
        final_out_filter0 = num_anchors * (5 + num_classes)  # num_classes是要检测的类的个数（我的数据集类别数是6）
        self.yolo_head1 = yolo_head([256, final_out_filter0], 128)  # 第三个yolo_head  （[2*输入，通道数]，输入）

    def forward(self, x):
        #  backbone
        x2, x1, x0 = self.backbone(x)  # x2是64的输出，x1是128的输出，x0是256的输出，
        ...
        return out0, out1, out2  # 256的预测结果，128的预测结果，64的预测结果
