...
from Pytorch.photo_detection.YOLO_v4.yolov4_pytorch_train.utils.utils import merge_bboxes
from Pytorch.photo_detection.YOLO_v4.yolov4_pytorch_train.nets.yolo_training import rand
import cv2
...
def yolo_dataset_collate(batch):  # 合并样本列表以形成小批量
    images = []
    targets = []
    for sample in batch:
        images.append(sample[0])
        targets.append(torch.FloatTensor(sample[1]))
    return torch.stack(images, 0), targets  # image是列表，list 中的每个元素为tensor 中第0维度的每个元素
    # 增加新的维度进行堆叠，若列表里面的维度是2，则就会变为3维
