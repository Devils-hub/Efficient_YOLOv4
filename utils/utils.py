...
class DecodeBox(nn.Module):  # 解码
    ...
    def forward(self, input):
        ...
        # 3代表3个先验框，1代表内部是否包含物体，4代表先验框的调整参数，num_classes代表先验框内部物体的种类
        # 一共多少张图片
        batch_size = input.size(0)  # 代表batch_size，把batch_size取出来
        # 13，13
        input_height = input.size(2)  # 特征层的高和宽
        ...
        # 416/13 = 32
        stride_h = self.img_size[1] / input_height  # 步长 = 输入图片的大小/特征层的宽高
        ...
        # 先验框的中心位置的调整参数
        x = torch.sigmoid(prediction[..., 0])  # 将预测的中心预测点移动到特征点的左上角
        ...
        # 生成网格，先验框中心，网格左上角的位置 batch_size,3,13,13
